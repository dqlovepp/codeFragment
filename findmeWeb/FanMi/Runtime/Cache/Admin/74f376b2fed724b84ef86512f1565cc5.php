    ...
    <div class="list-div" id="listDiv">
        <table cellpadding="3" cellspacing="1" >
            ...
            <?php if(is_array($brands)): $i = 0; $__LIST__ = $brands;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$brand): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td align="center">
                    <?php if(empty($brand["site_url"])): ?>未知
                    <?php else: ?>
                        <a href="<?php echo ($brand["site_url"]); ?>">
                        <?php if(mb_strlen($brand['site_url'],'utf-8') > 30): echo (msubstr($brand["site_url"],0,30,'utf-8',true)); ?>
                        <?php else: ?>
                        <?php echo ($brand["site_url"]); endif; ?> 
                        </a><?php endif; ?>
                </td>
                <td align="center">
                    <?php if(empty($brand["brand_desc"])): ?>为空
                    <?php else: ?>
                        <?php if(mb_strlen($brand['brand_desc'],'utf-8') > 40): echo (msubstr($brand["brand_desc"],0,40,'utf-8',true)); ?>
                        <?php else: ?>
                        <?php echo ($brand["brand_desc"]); endif; endif; ?>
                </td>
                <td align="center">
                <a href="/Admin/Brand/brandEdit/brandid/<?php echo ($brand["brand_id"]); ?>.html" title="编辑">编辑</a> |
                <a href="javascript:;" title="移除" onclick="delbrand(this,<?php echo ($brand["brand_id"]); ?>)">移除</a> 
                </td>
