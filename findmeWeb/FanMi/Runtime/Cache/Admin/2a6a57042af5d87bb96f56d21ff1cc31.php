...
<div class="main-div">
    <form action="/findmeWeb/Admin/Category/categoryAdd" method="post" name="addNavigate" id="addNavigate">
    <ul>
        <li>
            <label>上级分类:</label>
            <em>
                <select name="parent_id" style="height:30px; width:100px;">
                        <?php if(is_array($navigation)): foreach($navigation as $key=>$val): ?><option value="<?php echo ($val["nav_id"]); ?>" ><?php echo ($val["nav_name"]); ?></option><?php endforeach; endif; ?>
                    </select>
            </em>
        </li>
        <li>
            分类名称
            <em>
                <input type='text' name='cat_name' maxlength="20" value='' size='27' id='cat_name'/> 
            </em>
            <font color="red">*</font>
        </li>
        <li>
